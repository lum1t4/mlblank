...
from mlblank.core.utils import check_version, colorstr
...
def model_deparallel(model):
    """De-parallelize a model: returns single-GPU model if model is of type DP or DDP."""
    return model.module if model_is_parallel(model) else model
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        """Create EMA."""
        self.ema = deepcopy(model_deparallel(model)).eval()  # FP32 EMA
    ...
    def update(self, model):
        ...
        if self.enabled:
            ...
            msd = model_deparallel(model).state_dict()  # model state_dict
