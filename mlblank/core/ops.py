...
def segment2box(segment: torch.Tensor, width: int = 640, height: int = 640):
    """
...
def clip_coords(coords: torch.Tensor | np.ndarray, shape: tuple) -> torch.Tensor | np.ndarray:
    """
    Clip line coordinates to the image boundaries.

    Args:
        coords: A list of line coordinates.
        shape : A tuple of integers representing the size of the image in the format (height, width).
...
def scale_image(masks, im0_shape, ratio_pad=None) -> torch.Tensor:
    """
    ...
    Returns:
        masks : The masks that are being returned.
    """
...
def xywhn2xyxy(x: np.ndarray | torch.Tensor, w: int = 640, h: int = 640, padw: int = 0, padh: int = 0) -> np.ndarray | torch.Tensor:
    """
